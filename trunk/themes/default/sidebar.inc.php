		...
		<li class="lifestream_feedid_<?php echo $event->feed->id; ?> lifestream_feed_<?php echo $event->feed->get_constant('ID'); ?>" style="background-image: url('<?php echo $event->feed->get_icon_url(); ?>');">
			<div class="lifestream_text">
				...
				<?php if ($show_metadata) { ?>
					<div class="lifestream_meta">&mdash; <abbr title="<?php echo date("c", $event->timestamp); ?>" class="lifestream_hour"><?php echo $lifestream->timesince($event->timestamp); ?></abbr> <span class="lifestream_via">via <?php echo $event->get_feed_label() ?></span></div>
				<?php } ?>
