		...
		<table class="form-table">
			...
			<?php foreach ($options as $option=>$option_meta) { ?>
				...
				<tr>
					...
					<?php } else { ?>
						<th><label<?php if ($option_meta[1]) echo ' class="required"'; ?> for="id_<?php echo $option;?>"><?php echo htmlspecialchars($option_meta[0]);?></label></th>
						<td><input name="<?php echo $option;?>" type="<?php echo (str_endswith($option, 'password') ? 'password': 'text'); ?>" size="40" value="<?php echo htmlspecialchars($current_value); ?>">
						<?php if ($option_meta[4]) { ?>
						<div class="helptext"><?php echo $option_meta[4]; ?></div>
						<?php } ?></td>
					<?php } ?>
			...
			<tr id="id_icon_url_row">
				<th><label for="id_icon_url"><?php $lifestream->_e('Icon URL:'); ?></label><br /><small>(<?php $lifestream->_e('Optional'); ?>)</small></th>
				<td>
					<?php $current_value = (isset($_POST['icon_url']) ? stripslashes($_POST['icon_url']) : $instance->options['icon_url']); ?>
					<input type="text" id="id_icon_url" name="icon_url" value="<?php echo htmlspecialchars($current_value); ?>"/>
